...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            @include('qal.audit_khusus.informasi_anggota')
            ...
            <div class="col-md-7">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('qal.audit.khusus.tambah', $data_sampling_detail->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
        ...
        @include('qal.audit_khusus.modal_ketentuan')
        @include('qal.audit_khusus.modal_temuan_lain')
        @include('qal.audit_khusus.modal_ketentuan_temuan_lain')
...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <script>
        ...
        $('#formKetentuan').on('submit', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                url: "{{ route('qal.audit.khusus.ketentuan.store', ['id_ref_sampling' => $data_sampling_detail->id_ref_sampling, 'cif' => $data_sampling_detail->cif]) }}",
                method: "POST",
                data: formData,
        ...
        $('#formTemuanLain').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('qal.audit.khusus.temuan-lain.store', ['id_ref_sampling' => $data_sampling_detail->id_ref_sampling, 'cif' => $data_sampling_detail->cif]) }}",
                method: "POST",
                data: formData,
                success: function(response) {
        ...
        $(document).on('click', '.btnKetentuan', function() {
            ...
            $.ajax({
                url: "{{ route('qal.param.ketentuan.get', ':id') }}".replace(':id', paramId),
                method: 'GET',
                success: function(response) {
        ...
        function submitKetentuan() {
            ...
            fetch("{{ route('qal.audit.rutin.ketentuan.store', ['id_ref_sampling' => $data_sampling_detail->id_ref_sampling, 'cif' => $data_sampling_detail->cif]) }}", {
                    method: "POST",
                    headers: {
