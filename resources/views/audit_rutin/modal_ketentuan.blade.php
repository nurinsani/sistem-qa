                        ...
                        <form id="formKetentuan">
                            ...
                            @foreach ($groupedKetentuan as $subHeading => $items)
                                <div class="ketentuan-group {{ !$loop->first ? 'd-none' : '' }}"
                                    id="{{ Str::slug($subHeading) }}" data-page="1">

                                    @foreach ($items as $item)
                                        <div
                                            class="form-check mb-2 ketentuan-item {{ $loop->index >= 5 ? 'd-none' : '' }}">
                                    ...
                                    @if (count($items) > 5)
                                        <div
                                            class="sub-pagination d-flex justify-content-end align-items-center mt-3 bg-light p-2 rounded">
                                            <button type="button"
                                                class="btn btn-sm btn-outline-primary btn-sub-prev disabled"
                                                onclick="paginateGroup('{{ Str::slug($subHeading) }}', 'prev')">
                                                <i class="fas fa-chevron-left"></i>
                                            </button>
                                            <span class="page-info small mx-3 font-weight-bold">
                                                1 / {{ ceil(count($items) / 5) }}
                                            </span>
                                            <button type="button" class="btn btn-sm btn-outline-primary btn-sub-next"
                                                onclick="paginateGroup('{{ Str::slug($subHeading) }}', 'next')">
                            ...
                            <div class="d-flex justify-content-end align-items-center mt-4 pt-3 border-top">
                                <button type="button" class="btn btn-success" id="btnSimpanKetentuan"
